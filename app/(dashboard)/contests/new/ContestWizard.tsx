      ...
      const res = await fetch('/api/contests', {
        ...
        body: JSON.stringify({
          ...
          questions: questions.map(q => ({
            ...
            hackerrank_url: q.questionLink || q.hackerrank_url || `https://www.hackerrank.com/contests/${slug.trim()}/challenges/${q.slug}`,
          })),
  ...
  return (
    <div className="card" style={{ maxWidth: '820px', margin: '0 auto', padding: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
        {[{ n: 1, label: 'Scrape' }, { n: 2, label: 'Details & Review' }, { n: 3, label: 'Assign' }].map(s => (
          <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', opacity: step >= s.n ? 1 : 0.35, fontWeight: step === s.n ? 700 : 400, transition: 'all 0.3s' }}>
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: step >= s.n ? 'var(--accent)' : 'var(--border)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 700, transition: 'background 0.3s' }}>{s.n}</div>
        ...
        <div>
          ...
          <div style={{ background: 'var(--surface-2)', padding: '1.25rem', borderRadius: 10, border: '1px solid var(--border)', marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ fontWeight: 700, fontSize: '1rem' }}>📋 Scraped Questions ({questions.length})</h3>
            ...
            {questions.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '2rem 0', color: 'var(--text-muted)' }}>
                ...
                <p>No questions were scraped. Go back and try a different slug.</p>
              </div>
            ) : (
      ...
      {step === 3 && (
        <div>
          <div className="form-group" style={{ marginBottom: '1.5rem' }}>
            <label className="label" style={{ fontWeight: 700 }}>Assign to Groups</label>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.5rem' }}>
              {groups.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>No trainer groups created yet. Create groups under &quot;Groups&quot; tab first.</p>
              ) : groups.map(g => (
                <label key={g.id} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', cursor: 'pointer' }}>
          ...
          <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 8, padding: '0.75rem 1rem', marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
            <span><strong>Contest:</strong> {title}</span>

...
| 2026-08-29 | Antigravity Agent | **User Performance Profile Page (`/users/[userId]`)**: Built a dedicated developer-card-style performance profile for each user. (1) **Supabase RPC `get_user_performance_profile()`** (`supabase/migrations/11_user_performance_profile_rpc.sql`): Single DB-side aggregation returning user metadata, summary stats (total solved, total score, contests participated), LeetCode stats, batch-bounded activity heatmap data, and per-contest problem breakdowns with status/score/submission timestamps. (2) **API Route `GET /api/users/[userId]/profile`** with role-based access — trainers can only fetch their own profile, admins/managers can fetch any user. (3) **New page `app/(dashboard)/users/[userId]/page.tsx`** (server component) + **`UserProfileClient.tsx`** (client): Hero card with avatar, name, platform handle links, admin edit button; 4-stat summary grid (Problems Solved, Total Score, Contests Joined, LC Rating); batch-bounded GitHub-style activity heatmap; LeetCode stats with stacked difficulty bar; expandable per-contest problem table (status badge, score, last submission, problem link). (4) **`/profile` now redirects** to `/users/[id]` instead of rendering the old static profile page. (5) **Navigation entry points** added: Admin Users table (name is now a link), TrainerDetailModal header ("View Full Profile →" button), TopPerformersWidget cards and modal rows (clickable → profile), LeetCodeProgressWidget user modal ("View Full Profile →" button). Files: `supabase/migrations/11_user_performance_profile_rpc.sql`, `app/api/users/[userId]/profile/route.ts`, `app/(dashboard)/users/[userId]/page.tsx`, `app/(dashboard)/users/[userId]/UserProfileClient.tsx`, `app/(dashboard)/users/[userId]/page.css`, `app/(dashboard)/profile/page.tsx`, `app/(dashboard)/admin/users/UserTable.tsx`, `app/(dashboard)/contests/[id]/TrainerDetailModal.tsx`, `app/(dashboard)/dashboard/TopPerformersWidget.tsx`, `app/(dashboard)/dashboard/LeetCodeProgressWidget.tsx`. |

...
export function Sidebar({ role, mobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  const links = [
    ...
    {
      href: isManagerOrAdmin ? '/admin/roadmaps' : '/roadmaps',
        ...
        <nav className="sidebar-nav">
          {links.map((link) => {
            const isActive =
              ...
              (link.href === '/skills' && (pathname === '/courses' || pathname.startsWith('/courses/'))) ||
              (link.label === 'Topic Roadmaps' && (pathname.startsWith('/roadmaps') || pathname.startsWith('/admin/roadmaps')));
            return (
              <Link
                key={link.href}
                href={link.href}
